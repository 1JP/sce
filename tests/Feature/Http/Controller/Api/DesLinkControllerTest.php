...
class DesLinkControllerTest extends TestCase
{
    use RefreshDatabase;
     ...
     */
    public function test_unauthenticated_user_cannot_store_deslink(): void
    {
        $post = Post::factory()->create();
 
        $this->postJson(route('api.deslinks.store'), ['post_id' => $post->id])
            ->assertUnauthorized();
    }
    
    /**
     * Verifies that an authenticated user can successfully deslink a post,
     * receiving a 200 response with countPost = 1 and the record persisted in the database.
     */
    public function test_authenticated_user_can_deslink_a_post(): void
    {
        $post = Post::factory()->create();
 
        $response = $this->actingAs($this->user)
            ->postJson(route('api.deslinks.store'), ['post_id' => $post->id]);
    ...
    /**
     * Verifies that calling store on an already deslinked post toggles it off,
     * removing the record from the database and returning countPost = 0.
     */
    public function test_authenticated_user_can_undeslink_a_post(): void
    {
        $post = Post::factory()->create();
    ...
    /**
     * Ensures that countPost in the response reflects the total number of deslinks
     * on the post from all users, not just the authenticated one.
     */
    public function test_count_post_reflects_all_users_deslinks_on_post(): void
    {
        $post = Post::factory()->create();
 
        DesLink::factory()->count(3)->create([
            'post_id' => $post->id,
    ...
    /**
     * Verifies that an authenticated user can successfully deslink a comment,
     * receiving a 200 response with countComment = 1 and the record persisted in the database.
     */
    public function test_authenticated_user_can_deslink_a_comment(): void
    {
        ...
        $response = $this->actingAs($this->user)
            ->postJson(route('api.deslinks.store'), ['comment_id' => $comment->id]);
    ...
    /**
     * Verifies that calling store on an already deslinked comment toggles it off,
     * removing the record from the database and returning countComment = 0.
     */
    public function test_authenticated_user_can_undeslink_a_comment(): void
    {
        $comment = Comment::factory()->create([
            'post_id' => Post::factory()->create()->id,
            'user_id' => User::factory()->create()->id,
    ...
    /**
     * Ensures that countComment in the response reflects the total number of deslinks
     * on the comment from all users, not just the authenticated one.
     */
    public function test_count_comment_reflects_all_users_deslinks_on_comment(): void
    {
        $comment = Comment::factory()->create([
            'post_id' => Post::factory()->create()->id,
            'user_id' => User::factory()->create()->id,
        ...
        DesLink::factory()->count(2)->create([
            'comment_id' => $comment->id,
            'user_id'    => User::factory()->create()->id,
        ]);
    ...
    {
        $this->actingAs($this->user)
            ->postJson(route('api.deslinks.store'), ['post_id' => 'abc'])
            ->assertUnprocessable();
    }
    ...
    {
        $this->actingAs($this->user)
            ->postJson(route('api.deslinks.store'), ['comment_id' => 'abc'])
            ->assertUnprocessable();
    }
     ...
     * both on deslink and on subsequent undeslink, since no post interaction was performed.
     */
